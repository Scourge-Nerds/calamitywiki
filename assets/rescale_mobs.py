...
#                print(f"{tag_name} = {tag}: {value}")
        if isResized:
            ...
            output = item
            maxcolors = -1
            newscale = min(item.width, item.height)
            newwidth = int(item.width / newscale * 300)
            newheight = int(item.height / newscale * 300)
            try:
                ...
                output = output.resize(size= (newwidth, newheight), resample = 0 )
            except:
